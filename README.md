...
| **txt2img Log Relative Tolerance** | `10^x` relative tolerance (rtol) for txt2img, effective for adaptive solvers. |
...
| **Max ODE Steps** | Step count cap for adaptive solvers (shared across both passes). |
...
> personally runs txt2img and Hires.fix with deliberately different solvers (e.g.
> `euler` at a very high step count for txt2img, paired with `→ RK Sampler`'s
> `fe_kutta4` at a low step count for Hires.fix, sometimes even with a different
> checkpoint per pass), and wanted matching independent tolerance control to go
> with it.
...
|---|---|---|---|
| **Log Relative Tolerance** | −3.0 | −7.0 〜 0.0 | `10^x` rtol for adaptive solvers. Smaller = more precise, slower. Must be ≥ the Log Absolute Tolerance. Set independently for txt2img and Hires.fix in the Script accordion. |
| **Log Absolute Tolerance** | −4.0 | −7.0 〜 0.0 | `10^x` atol for adaptive solvers. Smaller = more precise, slower. Must be ≤ the Log Relative Tolerance. Set independently for txt2img and Hires.fix in the Script accordion. |
| **Max ODE Steps** | 250 | 1 〜 5000 | Failsafe step count cap for adaptive solvers. Has no effect on fixed-step solvers. Shared across both passes. |
...
```
TDE solver, TDE hires solver,
TDE log_rtol, TDE log_atol,
TDE hires log_rtol, TDE hires log_atol
```
...
| **txt2img Log Relative Tolerance** | txt2img の `10^x` 相対許容誤差（rtol）。可変ステップ法で有効。 |
...
| **Max ODE Steps** | 可変ステップ法のステップ数上限（両パス共通）。 |
...
> txt2img と Hires.fix で意図的に異なるソルバーを使い分けている（例: txt2img は
> `euler` を非常に高いステップ数で、Hires.fix は `→ RK Sampler` 経由の `fe_kutta4`
> を低いステップ数で、場合によってはパスごとにチェックポイントも切り替える）ため、
> それに合わせて許容誤差も独立して制御できるようにと付けた機能です。
...
| パラメータ | デフォルト | 範囲 | 説明 |
|---|---|---|---|
| **Log Relative Tolerance** | −3.0 | −7.0 〜 0.0 | 可変ステップソルバーの `10^x` rtol。小さいほど高精度・低速。Log Absolute Tolerance 以上にする。Script アコーディオンで txt2img と Hires.fix を独立に設定可能。 |
| **Log Absolute Tolerance** | −4.0 | −7.0 〜 0.0 | 可変ステップソルバーの `10^x` atol。小さいほど高精度・低速。Log Relative Tolerance 以下にする。Script アコーディオンで txt2img と Hires.fix を独立に設定可能。 |
| **Max ODE Steps** | 250 | 1 〜 5000 | 可変ステップソルバーのステップ数上限（フェイルセーフ）。固定ステップ法には影響しない。両パス共通。 |
